...
// Lazy JWKS initializer to avoid evaluating undefined envs at build time
let jwksFn: ReturnType<typeof createRemoteJWKSet> | null = null;
function getJwks(): ReturnType<typeof createRemoteJWKSet> {
  if (!jwksFn) {
    const url = process.env.ABHA_JWKS_URL;
    if (!url) {
      throw new Error('ABHA_JWKS_URL is not set');
    }
    jwksFn = createRemoteJWKSet(new URL(url));
  }
  return jwksFn;
}
...
export async function verifyAbhaJwt(token: string, requiredScopes: string[] = []): Promise<AbhaClaims> {
  ...
  try {
    const issuer = process.env.ABHA_ISSUER;
    const audience = process.env.ABHA_AUDIENCE;
    if (!issuer || !audience) {
      throw new AbhaVerificationError('ABHA_ISSUER or ABHA_AUDIENCE not set', 'config_error');
    }
